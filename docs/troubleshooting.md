...
For a source checkout, run `pnpm build` before `node packages/mcp/bin/openapi-to-mcp.js`. Confirm Node.js is 22.12 or newer for repository commands, and that the Host starts the process with the intended Workspace as its working directory. Published packages retain a Node.js 22 or newer runtime floor.
