...
Aggregate package 提供 `pluginTSType`、`pluginZod`、`pluginTSRequest`、`pluginReactQuery`、`pluginSWR`、`pluginVueQuery` 和 `pluginMSW`；随后以 `strict: true` 编译 generated TypeScript、React Query consumer usage 与 consumer request stub。TS 6 和 TS 7 使用完整的外部 declaration checking；TS 5.6 使用同一 strict generated consumer 加 `--skipLibCheck`，仅跳过当前 MSW → `type-fest@5.9.0` 声明中 TS 5.6 不认识的 `Float16Array`，并在 report 中显式记录该 option。该 compatibility boundary 不放宽 generated source 的 strict checking。独立 runtime entry 使用 Zod 4 执行 generated model、request body、query、path、response、boolean/empty 与 recursive schema，并检查 accepted/rejected value。Compile-time assertion 锁定 ordinary model/response schema 的 precise inference，以及 recursive schema 已文档化的 `unknown` inference boundary。普通 aggregate-only consumer 的 release smoke 仍不安装 React/TanStack runtime。
