...
| Generate bare API path / METHOD path first-attempt conformance | Supervised/manual real Codex acceptance（Issue #149；复用 Issue #113 的既有 acceptance mechanism） | repository contract、static evaluation matrix、`release:smoke` packaged Skill verification | Yes | Yes | Host-dependent | 在 Consumer 中输入 `/pet/findByStatus` 与 `GET /pet/findByStatus`，验证 Skill 首次激活、按实际 MCP capability 执行有界 Target/search/exact candidate/contract discovery，并在只读结果后停止；明确实现请求另行验证进入既有 generation workflow。Frontend route/no-match、歧义与 capability 降级须 fail closed。不得用静态 evaluation、repository contract 或 packed bytes 冒充 real-Agent routing PASS。 |
