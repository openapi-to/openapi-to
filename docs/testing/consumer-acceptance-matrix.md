...
| Generated TypeScript compile | `test:consumer:codegen` | `release:smoke` | Yes | Yes | Local/CI host | 同一 generated consumer 运行 TS 5.6、6、7 matrix；三者保持 `strict: true`，TS 5.6 额外使用 `--skipLibCheck` 以隔离当前 MSW/type-fest 外部声明的 `Float16Array` 差异。 |
...
| SWR consumer contract | `test:consumer:codegen` | `packages/plugin-swr` focused tests, `release:smoke` | Yes | Yes | Local/CI host | Packed SWR generation 以 strict consumer request/error contract 检查 fetcher、AbortSignal 和无 `any`/`ts-ignore`。 |
