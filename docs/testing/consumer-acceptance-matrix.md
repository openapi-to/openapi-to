...
| Setup Inspector ↔ packed MCP Developer/Read-only/Hardened agreement | `release:smoke` bridge | `openapi-to-setup.node-test`, MCP tests | Yes | Yes | Release-smoke platform | Repository-Skill Inspector 必须区分 Developer 与 Read-only 的 8-Tool Schema，并验证 Hardened 的 Prepare/Apply；等价的 packed MCP command 必须暴露匹配的 current Schemas。 |
