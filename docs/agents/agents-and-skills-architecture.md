...
Use one primary workflow for a task. Development Issue creation, refinement,
audit, readiness, blocking/recovery, and close/reopen use
`manage-development-issue` as the specialized primary. General repository changes use
...
implementation, PR feedback repair, CI repair, or execution authority.
...
| `plan-development-wave` | Bounded multi-Development-Issue planning for Dependency DAG, Current WIP, Shared Surface, Execution Frontier, recommended wave, serialized integration, and revalidation | Added as a read-only planner. It uses verified Issue/PR/current-main/CI facts, keeps discovery bounded, and never mutates Issues, Projects, repository files, or execution state. |
