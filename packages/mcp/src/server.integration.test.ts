...
`,
    ...
    expect(structured(outdated)).toMatchObject({ success: false, outdated: false })
    expect((structured(outdated).diagnostics as Array<{ code: string }>).map(({ code }) => code)).toContain('OUTPUT_MANAGED_PATH_CHANGED')
