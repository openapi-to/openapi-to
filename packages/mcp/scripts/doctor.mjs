...
async function runDoctor(checks, state) {
  ...
  try {
    ...
    await runCheck(checks, 'selective-prepare-no-write', async () => {
      ...
      assert(await missing(path.join(workspaceRoot, '.openapi-to/generation-intents')), 'Selective Prepare created the Generation Intent directory.')
    })
    await runCheck(checks, 'selective-apply', async () => {
      ...
      const selectionDirectory = path.join(workspaceRoot, '.openapi-to/generation-intents')
      ...
      assert(equalValues(selection.scope?.operationKeys, ['listPets']), 'Selective Apply committed unexpected Generation Intent operations.')
